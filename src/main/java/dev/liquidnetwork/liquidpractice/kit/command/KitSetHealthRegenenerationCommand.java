...
@CommandMeta(label={"kit healthregen", "kit healthregeneration"}, permission = "practice.admin")
public class KitSetHealthRegenenerationCommand {
    public void execute(Player player, @CPL("kit") Kit kit) {
        if (kit == null) {
            ...
            kit.save();
            player.sendMessage(CC.translate("&7Updated health-regeneration mode for &b" + kit.getName() +  " &7to &b" + (kit.getGameRules().isHealthRegeneration() ? "true!" : "false!")));
        }
    }
}